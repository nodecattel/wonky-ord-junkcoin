...
use crate::jkc20::deploy::Deploy;
use crate::jkc20::errors::JSONError;
use crate::jkc20::mint::Mint;
use crate::jkc20::OperationType;
use crate::jkc20::params::PROTOCOL_LITERAL;
...
pub(crate) fn deserialize_jkc20_operation(
  ...
  let raw_operation = match deserialize_jkc20(content_body) {
    Ok(op) => op,
    Err(e) => {
      return Err(e.into());
    }
  ...
  match action {
    ...
    Action::Transfer => match raw_operation {
      ...
      _ => Err(JSONError::NotJKC20Json.into()),
    },
  }
}

fn deserialize_jkc20(s: &str) -> Result<RawOperation, JSONError> {
  ...
  if identifier != Some(&json!(PROTOCOL_LITERAL)) {
    return Err(JSONError::NotJKC20Json);
  }
