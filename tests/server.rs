...
fn inscription_appears_on_reveal_transaction_page() {
  ...
  TestServer::spawn_with_args(&rpc_server, &[]).assert_response_regex(
    format!("/tx/{reveal}"),
    format!(".*<h1>Transaction .*</h1>.*<a href=/Junkscription/{reveal}.*"),
  );
}
...
fn inscription_appears_on_output_page() {
  ...
  TestServer::spawn_with_args(&rpc_server, &[]).assert_response_regex(
    ...
    format!(".*<h1>Output <span class=monospace>{reveal}:0</span></h1>.*<a href=/Junkscription/{inscription}.*"),
  );
}
...
fn inscription_page_after_send() {
  ...
  ord_server.assert_response_regex(
    format!("/Junkscription/{inscription}"),
    format!(
      r".*<h1>Junkscription 0</h1>.*<dt>location</dt>\s*<dd class=monospace>{reveal}:0:0</dd>.*",
    ),
  );
  ...
  ord_server.assert_response_regex(
    format!("/Junkscription/{inscription}"),
    format!(
      r".*<h1>Junkscription 0</h1>.*<dt>address</dt>\s*<dd class=monospace>bc1qcqgs2pps4u4yedfyl5pysdjjncs8et5utseepv</dd>.*<dt>location</dt>\s*<dd class=monospace>{send}:0:0</dd>.*",
    ),
  )
}
...
<div class=thumbnails>
  <a href=/Junkscription/{inscription}><iframe .*></a>
</div>.*",
    ),
  );
}
...
fn inscriptions_page() {
  ...
  TestServer::spawn_with_args(&rpc_server, &[]).assert_response_regex(
    ...
    format!(
      ".*<h1>Junkscription</h1>
<div class=thumbnails>
  <a href=/Junkscription/{inscription}>.*</a>
</div>
.*",
    ),
  );
...
fn inscriptions_page_is_sorted() {
  ...
  for _ in 0..8 {
    let Inscribe { inscription, .. } = inscribe(&rpc_server);
    inscriptions.insert_str(0, &format!(".*<a href=/Junkscription/{inscription}>.*"));
  }

  TestServer::spawn_with_args(&rpc_server, &[])
    .assert_response_regex("/Junkscriptions", &inscriptions);
}
...
fn inscriptions_page_has_next_and_previous() {
  ...
  TestServer::spawn_with_args(&rpc_server, &[]).assert_response_regex(
    format!("/Junkscription/{b}"),
    format!(
      ".*<h1>Junkscription 1</h1>.*
<div class=inscription>
<a class=prev href=/Junkscription/{a}>❮</a>
<iframe .* src=/preview/{b}></iframe>
<a class=next href=/Junkscription/{c}>❯</a>
</div>.*",
    ),
  );
}
